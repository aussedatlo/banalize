...
export const CORE_URL =
  process.env.BANALIZE_E2E_CORE_URL ?? "http://localhost:6042";

/** UI base URL (Caddy, proxies /api to the core). */
export const UI_URL =
  process.env.BANALIZE_E2E_UI_URL ?? "http://localhost:6043";
...
export const LOG_DIR =
  process.env.BANALIZE_E2E_LOG_DIR ??
  path.join(REPO_ROOT, "apps", "e2e", ".tmp", "logs");

/** Path of a watched log file as the core sees it inside the container. */
export const containerLogPath = (file: string): string =>
  `/var/log/banalize-e2e/${file}`;
